...
void *arena_realloc(Arena *arena, void *ptr, size_t initial_size,
...
#define make(T, parent_arena)                                                  \
  (T##Slice) {                                                                 \
    .arr = (T *)arena_alloc(parent_arena, sizeof(T) * 8), .alloc_len = 8,      \
    .len = 0, .arena = parent_arena                                            \
  }
#define array(T) T.arr
#define append(v, q)                                                           \
  if (v.len + 1 > v.alloc_len) {                                               \
    v.arr = (typeof(array(v)))arena_realloc(                                   \
        v.arena, v.arr, v.alloc_len * sizeof(v.arr[0]),                        \
        v.alloc_len * 2 * sizeof(v.arr[0]));                                   \
    v.alloc_len *= 2;                                                          \
    array(v)[v.len] = q;                                                       \
    v.len++;                                                                   \
  } else {                                                                     \
...
#define resize(v, q)                                                           \
  if (v.alloc_len < q) {                                                       \
    ...
    v.arr = (typeof(v.arr))arena_realloc(                                      \
        v.arena, v.arr, v.alloc_len * sizeof(v.arr[0]), l * sizeof(v.arr[0])); \
    v.alloc_len = q;                                                           \
  }                                                                            \
  if (q < v.len) {                                                             \
    v.len = q;                                                                 \
...
#define clone(_arena, slice)                                                   \
  (typeof(slice)){                                                             \
      .arr = mem_clone(_arena, slice.arr, sizeof(slice.arr[0]), len(slice)),   \
      .len = slice.len,                                                        \
      .alloc_len = slice.alloc_len,                                            \
      .arena = _arena};
...
#define str_concat(a, b) _strconcat(&a, (const char *)b, sizeof(b[0]));
...
void *arena_realloc(Arena *arena, void *ptr, size_t initial_size,
  ...
  if (ptr == arena->last_allocation) {
    if ((size_t)(arena->end) - (size_t)(arena->last_allocation) >=
        requested_size) {
      arena->ptr = arena->last_allocation + requested_size;
      if ((size_t)(arena->ptr) % 8 != 0) {
        arena->ptr =
            (void *)((size_t)arena->ptr + 8 - ((size_t)(arena->ptr) % 8));
      }
